...
class GoodsService
{
    ...
    public static function buildGoodsList(&$goodsList)
    {
        ...
        //语言处理
        $languageWhere = ['status' => '1', 'is_deleted' => '0'];
        $languageField = 'id,language';
        $languageList = Db::name('StoreLanguage')->where($languageWhere)->order('sort asc,id desc')->column($languageField);   
        
        // 无商品列表时
        if (empty($goodsList)) {
            return ['list' => $goodsList, 'cate' => $cateList, 'brand' => $brandList,'language'=>$languageList];
        }
        // 读取商品详情列表
